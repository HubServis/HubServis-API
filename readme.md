...
Separa Ecommerce/Gestão

Gestão (
config dos funcionários(Estabelecimento){horários, pagementos, eventos?},
sistema de pagemanetos(Estebelecimento -> Usuário){após feito serviço, será feito handling de pagamento? Ou durante agendamento, já pagou?},
parte fiscal(Estabelecimento){saldos, orçamentos, dívidas, receita fiscal?},
agendamento(Usuário<->Estabelecimento){aqui já paga?}

    clientes grandes(permissão de fazer layouts e até editar dados de comércio?){
        customizar a ideia do plano -> num cadastros, perfíl, preços, etc...
    }
...
> Ecommerce (
    ...
    Galeria de produtos(Pagamento inline? Api?),
    Serviços -> linka pra parte de agendamento(Cliente),
