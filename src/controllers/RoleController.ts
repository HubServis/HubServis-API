...
const createRolePermissionService = new CreateRolePermissionService(new RolePermissionRepositorySqlite());
...
class RoleController implements IRoleCotroller{
    ...
    async createRolePermission(req: Request, res: Response){
        ...
        try {
            const result = await createRolePermissionService.execute({ permissions, roleId });
            console.log(result);
